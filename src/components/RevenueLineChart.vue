...
/**
 * RevenueLineChart
 * Robust: lädt pro Ticker, ignoriert Fehlschläge, zeigt nur valide Datensätze.
 */
...
export default {
  ...
  async created() {
    try {
      ...
      if (!ok.length) { this.error = 'No revenue series available'; return; }
      const labels = lastN(ok[0].quarters, 12);
      this.chartData = this.buildData(ok, labels);
      this.options = this.buildOptions();
      this.ready = true;
    } catch { this.error = 'Failed to load revenue series'; }
  },
  methods: {
    /** Load each ticker individually; keep only fulfilled. */
    async loadAll(tickers) {
      ...
      return results.map((r, i) => {
        if (r.status !== 'fulfilled') { console.warn('load failed', tickers[i]); return null; }
        return { ticker: r.value.ticker || tickers[i], quarters: r.value.quarters, revenue: r.value.revenue };
      }).filter(Boolean);
    },
    /** Build dataset config for Chart.js. */
    ...
    /** Chart options with shared tooltip. */
    buildOptions() {
      return {
        responsive: true,
        ...
        plugins: {
          ...
          tooltip: {
            callbacks: {
              ...
              label: i => ` ${i.dataset.label}: ${fmtBillionsUSD(Number.isFinite(i.parsed.y) ? i.parsed.y : null)}`
            }
          }
        },
        scales: {
          x: { grid: { display: false } },
          y: { grid: { color: 'rgba(255,255,255,0.08)' }, ticks: { callback: v => `$${Number(v).toFixed(0)}B` } }
        }
      };
    }
  }
