...
export default {
  ...
  data() {
    return {
      company: null,       /** @type {null | {quarters:string[], revenue:(number|null)[]}} */
      error: ''
    };
  },
  computed: {
  ...
  methods: {
    /**
     * Return index of last finite value in series.
    ...
    /**
     * Current and previous revenue numbers.
     * @returns {[number|null, number|null]}
     */
    currPrev() {
      if (!this.company) return [null, null];
      const i = this.lastIdx, p = Math.max(0, i - 1);
...
.quarter { opacity:.8; font-size:12px; }
.row { display:flex; gap:24px; margin-top:12px; }
.kpi-label { font-size:12px; opacity:.8; }
.kpi-value { font-size:24px; }
.delta { margin-right:8px; }
.err { color:#f66; margin-top:12px; }
</style>
